...
The built plugin will be available in the `dist/` directory as `EntityHighlight.js`.
...
Example-Plugin/
├── src/
│   ├── EntityHighlight.ts    # Main plugin class
...
1. **Rename your plugin**: Update the `pluginName` and `author` properties in `src/EntityHighlight.ts`
...
3. **Copy the built plugin**:
   ...
   ```bash
   cp dist/EntityHighlight.js /path/to/HighLiteDesktop/src/renderer/client/highlite/plugins/
   ```
